...
void CBuffer::StartSyntaxParse()
{
	...
	DWORD dwID;
	// CJC don't use default stack size as POVWIN has it set fairly large to handle recursion during tracing
	m_hSyntaxThread = CreateThread( NULL, 1024*1024, ( LPTHREAD_START_ROUTINE ) ::SyntaxParserThread, this, 0, &dwID );
	SetThreadPriority (m_hSyntaxThread, THREAD_PRIORITY_ABOVE_NORMAL) ;
	ASSERT( m_hSyntaxThread );
}
