 ...
 * ---------------------------------------------------------------------------
 * $File: //depot/povray/3.5/windows/codemax/cmedit/settings.cpp $
 * $Revision: #2 $
 * $Change: 4779 $
...
void GetSettings (CRegDef *t, EditConfigStruct *ec, bool RestoreFiles)
{
  ...
  if (RestoreFiles)
  {
    ...
    for (int i = 0 ; i < MAX_EDITORS ; i++)
    {
      ...
      if (params == "" || params [1] == ',')
        continue ;
      if ((e = CreateNewEditor (GetNextField (params), false, false)) == NULL)
        continue ;
      ...
      e->SetTabSize (atoi (GetNextField (params))) ;
      e->SetAutoIndent ((TAutoIndent) atoi (GetNextField (params))) ;
    }
    delete openReg ;
  }
...
void PutSettings (CRegDef *t, EditConfigStruct *ec)
{
  ...
  t->WriteBin ("Colours", (char *) &ec->Colours, sizeof (CM_COLORS)) ;
  t->WriteBin ("FontStyles", (char *) &ec->FontStyles, sizeof (CM_FONTSTYLES)) ;
