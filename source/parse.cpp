...
static void Parse_Camera (CAMERA **Camera_Ptr)
{
	...
	else if (opts.Language_Version >= 350)
	{
		...
		EXPECT
			// ***Yu Ji*** //
			CASE (LIGHTFIELD_SLABS_TOKEN)
				New->Type = LIGHT_FIELD_SLABS;
			END_CASE
			...
			case LIGHT_FIELD_SLABS:
				EXPECT
					...
					CASE2(SPHERICAL_TOKEN, CYLINDER_TOKEN)
						Expectation_Error("light field camera modifier");
					END_CASE
	...
	else // old style syntax
	{
		EXPECT
			...
			CASE (LIGHT_FIELD_SUBIMG_HEIGHT_TOKEN)
			New->cam_num_height = Parse_Float();
				
			END_CASE

			CASE (LIGHT_FIELD_SUBIMG_WIDTH_TOKEN)
			New->cam_num_width = Parse_Float();
				
			END_CASE

			CASE (LIGHT_FIELD_SLAB_LENGTH_TOKEN)
			New->slab_length = Parse_Float();
				
			END_CASE

			CASE (LIGHT_FIELD_SLAB_LENGTH_EX_TOKEN)
			New->slab_length_ex = Parse_Float();
				
			END_CASE

			CASE (LIGHT_FIELD_SLAB_HEIGHT_TOKEN)
			New->slab_height = Parse_Float();
				
			END_CASE

			CASE (LIGHT_FIELD_SLAB_HEIGHT_EX_TOKEN)
			New->slab_height_ex = Parse_Float();
