...
#define FCL Frame.Camera->Location
  ...
  /* Create primary ray according to the camera used. */
   ...
   //  *** Start Modified by Yu Ji *** // 
  int sub_cam_height, sub_cam_width, cam_idx_height, cam_idx_width, cam_face_num;
  DBL y_inv;
  VECTOR cam_1st_loc, cam_height_shift, cam_width_shift;
  VECTOR sub_cam_dir, sub_cam_up, sub_cam_right;

  int tt;
  if(Frame.Camera->Type == LIGHT_FIELD_SLABS)
  {
	  sub_cam_height = Frame.Screen_Height/(4*Frame.Camera->cam_num_height);
	  sub_cam_width = Frame.Screen_Width/Frame.Camera->cam_num_width;

	  cam_idx_width = (int)(x/(DBL)sub_cam_width);
	  y_inv = ((DBL)(Frame.Screen_Height - 1) - y);
	  cam_face_num = (int)(y_inv*4.0/(DBL)Frame.Screen_Height);
	  cam_idx_height = (int)(((DBL)(y_inv-cam_face_num*Frame.Camera->cam_num_height*sub_cam_height))/(DBL)sub_cam_height);

	  switch(cam_face_num)
	  {
	  case FACE_FRONT:
		  cam_1st_loc[X] = Frame.Camera->Location[X]-Frame.Camera->slab_length_ex/2.0;
		  cam_1st_loc[Y] = Frame.Camera->Location[Y]+Frame.Camera->slab_length/2.0;
		  cam_1st_loc[Z] = Frame.Camera->Location[Z]-Frame.Camera->slab_height/2.0;

		  cam_width_shift[X] = Frame.Camera->slab_length_ex/(Frame.Camera->cam_num_width-1.0) ;
		  cam_width_shift[Y] = 0.0;
		  cam_width_shift[Z] = 0.0;
		  cam_height_shift[X] = 0.0;
		  cam_height_shift[Y] = 0.0;
		  tt = Frame.Camera->slab_height;
		  cam_height_shift[Z] = Frame.Camera->slab_height/(Frame.Camera->cam_num_height-1.0 +0.000000000000001);
		  ...
		  break;
	  case FACE_RIGHT:
		  cam_1st_loc[X] = Frame.Camera->Location[X]+Frame.Camera->slab_length/2.0;
		  cam_1st_loc[Y] = Frame.Camera->Location[Y]+Frame.Camera->slab_length_ex/2.0;
		  cam_1st_loc[Z] = Frame.Camera->Location[Z]-Frame.Camera->slab_height/2.0;

		  cam_width_shift[X] = 0.0;
		  cam_width_shift[Y] = -Frame.Camera->slab_length_ex/(Frame.Camera->cam_num_width-1.0);
		  cam_width_shift[Z] = 0.0;
		  cam_height_shift[X] = 0.0;
		  cam_height_shift[Y] = 0.0;
		  cam_height_shift[Z] = Frame.Camera->slab_height/(Frame.Camera->cam_num_height-1.0 +0.000000000000001);
	  ...
	  VLinComb3(Ray->Initial, 1.0, cam_1st_loc, (DBL)cam_idx_width, cam_width_shift, (DBL)cam_idx_height, cam_height_shift);
  ...
  switch (Frame.Camera->Type)
  {
	 ...
	 *  ***Yu Ji*** 
     */
  case LIGHT_FIELD_SLABS:

	  x0 = (x - (DBL)cam_idx_width*(DBL)sub_cam_width)/(DBL)sub_cam_width * 2.0 - 1.0;
	  tt = Frame.Camera->cam_num_height;
	  y0 = (y_inv - (DBL)cam_face_num*(DBL)Frame.Camera->cam_num_height*(DBL)sub_cam_height - (DBL)cam_idx_height*(DBL)sub_cam_height)/(DBL)sub_cam_height * 2.0 - 1.0;

	  printf("y0 = %f\n",y0);
	  VLinComb3(Ray->Direction, 1.0, sub_cam_dir, x0, sub_cam_right, y0, sub_cam_up);
